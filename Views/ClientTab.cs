...
namespace Flintstones
{
  public class ClientTab : TabPage
  {
    ...
    {
      string str = this.saveFileName(this.newtemplate_name.Text);
    ...
    /// <summary>
    /// Removes common special characters from the specified string and convert to lower case so it is safe to use as a filename.
    /// </summary>
    /// <remarks>The method removes characters such as commas, periods, slashes, exclamation marks,
    /// and other common punctuation. The operation is case-sensitive and does not modify letters or
    /// digits.</remarks>
    /// <param name="str">The input string from which to remove special characters. Cannot be null.</param>
    /// <returns>A new string with all occurrences of common special characters removed. If no special characters are
    /// present, returns the original string.</returns>
    public string saveFileName(string input)
    {
      string safeName = string.Concat(input.Split(Path.GetInvalidFileNameChars()));
    ...
    public void SaveTemplate(string toSave = "", bool OnDC = false)
    {
      ...
      // Save client tasks settings in Tasks section
      // TODO: Add prayer settings
      doc.Root.Add(new XElement("Tasks",
          new XElement("AutomaticAssist", assistonthischar.Checked),
          new XElement("AscensionRescueEnabled", vrescueascender),
          new XElement("AscensionRescueName", vrescueascendername),
          new XElement("Profession", improveskill.SelectedItem),
          new XElement("SkillAssistantEnabled", useskillassistant.Checked),
          new XElement("SkillAssistantName", skillassistant.Text),
          new XElement("ProfessionRestartEnabled", impskillinresponse.Checked),
          new XElement("ProfessionRestartCommand", impskillinresponsetext.Text),
          new XElement("RequestLaborEnabled", requestlabor.Checked),
          new XElement("RequestLaborFrom", requestlabornametext.Text),
          new XElement("RequestLaborCommand", requestlabormessagetext.Text),
          new XElement("LaborFor", laborname.Text),
          new XElement("LaborDays", labordays.Value),
          new XElement("LaborResponseEnabled", laborinresponse.Checked),
          new XElement("LaborResponseCommand", laborinresponsetext.Text),
          new XElement("LaborWhisperDoneEnabled", laborwhisper.Checked),
          new XElement("LaborWhisperDoneCommand", laborwhispertext.Text),
          new XElement("LaborLogOff", laborlogoff.Checked)
      ));
      ...
      // Save template/settings to file on a DC or by user input
      try
      {
        string clientSettingsPath = Program.StartupPath + "\\Settings\\" + this.Client.Name.ToLower();
        if (OnDC)
        {
          string dcSettingsFile = clientSettingsPath + "\\" + this.Client.Name.ToLower() + "_dc.xml";
          doc.Save(dcSettingsFile);
        }
        else
        {
          string lower = string.IsNullOrEmpty(toSave) ? saveFileName(newtemplate_name.Text) : saveFileName(toSave);
          if (!string.IsNullOrEmpty(lower))
          {
            string clientSettingsFile = clientSettingsPath + "\\" + lower + ".xml";
      ...
      { 
        this.Client.SendMessage("Error saving template: " + ex.Message, "red"); 
      }
    ...
    public void LoadTemplate(string toLoad = "", bool AfterRelog = false)
    {
      ...
      if (Directory.Exists(Program.StartupPath + "\\Settings\\" + this.Client.Name.ToLower()))
      {
        if (File.Exists(Program.StartupPath + "\\Settings\\" + this.Client.Name.ToLower() + "\\" + str1 + ".xml"))
        {
          ...
          {
            // Read values safely
            assistonthischar.Checked = bool.TryParse(tasks.Element("AutomaticAssist")?.Value, out var a1) ? a1 : false;
            vrescueascender = bool.TryParse(tasks.Element("AscensionRescueEnabled")?.Value, out var a2) ? a2 : false;
            vrescueascendername = tasks.Element("AscensionRescueName")?.Value ?? "";
            improveskill.SelectedItem = tasks.Element("Profession")?.Value ?? "";
            useskillassistant.Checked = bool.TryParse(tasks.Element("SkillAssistantEnabled")?.Value, out var a3) ? a3 : false;
            skillassistant.Text = tasks.Element("SkillAssistantName")?.Value ?? "";
            impskillinresponse.Checked = bool.TryParse(tasks.Element("ProfessionRestartEnabled")?.Value, out var a4) ? a4 : false;
            impskillinresponsetext.Text = tasks.Element("ProfessionRestartCommand")?.Value ?? "";
            requestlabor.Checked = bool.TryParse(tasks.Element("RequestLaborEnabled")?.Value, out var a5) ? a5 : false;
            requestlabornametext.Text = tasks.Element("RequestLaborFrom")?.Value ?? "";
            requestlabormessagetext.Text = tasks.Element("RequestLaborCommand")?.Value ?? "";
            laborname.Text = tasks.Element("LaborFor")?.Value ?? "";
            labordays.Value = int.TryParse(tasks.Element("LaborDays")?.Value, out var i1) ? i1 : 0;
            laborinresponse.Checked = bool.TryParse(tasks.Element("LaborResponseEnabled")?.Value, out var a6) ? a6 : false;
            laborinresponsetext.Text = tasks.Element("LaborResponseCommand")?.Value ?? "";
            laborwhisper.Checked = bool.TryParse(tasks.Element("LaborWhisperDoneEnabled")?.Value, out var a7) ? a7 : false;
            laborwhispertext.Text = tasks.Element("LaborWhisperDoneCommand")?.Value ?? "";
            laborlogoff.Checked = bool.TryParse(tasks.Element("LaborLogOff")?.Value, out var a8) ? a8 : false;
...
"y";
      ...
      this.useexpgem.Text = "Use Exp Gem at MaxHP Box";
